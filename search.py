...
class search_functions:
    ...
    def search_session(self, search_name):
        response = requests.get(str(self.url) + str(search_name) + str('/session'))
        content = response.content
        ...
        return extractor, gametype, clan_tag #Return stats, gamemodes, and squadrom
    ...
    def search_vehicles(self, search_name, game_mode, search_vehicle):
        ...
        for item in indexes:
            if (re.sub("[^.0-9]","", text[item + 1])) == '0': #Delete vehicles that are not played by looking at the battles number, which comes right after the vehicle
                del indexes[indexes.index(item)]
        i = 0
        ...
        for item in extractor: 
            ...
            if j == 2: 
                ...
                kbg = round(kbg, 2) #Calculate kda, kb
                if k == 1: #Set deaths back to 0
                    stats[-3] = 0
                    k = 0
                winrate = float((stats[-5]/stats[-6]) * 100) 
                ...
                j = 0
        return stats, vehicles, clan_tag #Return the stats, the vehicles played, and the squadron